...
impl ComponentState {
    ...
    pub(super) fn lift_function(
        ...
        {
            return Err(BinaryReaderError::new(
                format!("lowered function parameters `{:?}` do not match parameter types `{:?}` of core function {func_index}", params.as_slice(), core_ty.params),
                offset,
            ));
        }
        ...
        if core_ty.returns.as_ref() != results.as_slice() {
            return Err(BinaryReaderError::new(
                format!("lowered function results `{:?}` do not match result types `{:?}` of core function {func_index}", results.as_slice(), core_ty.returns),
                offset,
            ));
        }
